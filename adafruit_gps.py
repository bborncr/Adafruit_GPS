    ...
    def _update_timestamp_utc(self, time_utc, date=None):
        ...
        if date is None:
            if self.timestamp_utc is None:
                ...
                day = time.gmtime(self.timestamp_utc)[2]
                month = time.gmtime(self.timestamp_utc)[1]
                year = time.gmtime(self.timestamp_utc)[0]
...
#                 year = self.timestamp_utc.tm_year
        ...
        self.timestamp_utc = time.mktime(
            (year, month, day, hours, mins, secs, 0, 0)
        )
...
class GPS_GtopI2C(GPS):
    ...
    def __init__(
        self, i2c, *, address=_GPSI2C_DEFAULT_ADDRESS, debug=False, timeout=5000
    ):
    ...
    def read(self, num_bytes=1):
        ...
        for _ in range(num_bytes):
            ...
            self._i2c.readfrom_into(self._addr, self._charbuff)
    ...
    def write(self, bytestr):
        ...
        or checksums"""
        #i2c.write(bytestr)
        self._i2c.writeto(self._addr, bytestr)
    ...
    def readline(self):
        """Returns a newline terminated bytearray, must have timeout set for
        the underlying UART or this will block forever!"""
        timeout = time.ticks_ms() + self._timeout
